...
export class ProductExtractionWorkflow {
  ...
  async downloadAndExtractFrames(
    state: GraphStateType
  ): Promise<Partial<GraphStateType>> {
    try {
      console.log("Step 1: Downloading video and extracting frames...");

      const videoPath = await this.videoProcessor.downloadYouTubeVideo(
        state.videoUrl
      );
      console.log(`Video downloaded to: ${videoPath}`);

      // Extract frames at various intervals
      const framePaths = await this.videoProcessor.extractFrames(
        videoPath,
        [2, 5, 10, 15, 20, 30, 45]
      );
    ...
    try {
      ...
      for (const framePath of framePaths) {
        ...
        const detectedProducts =
          await this.geminiClient.analyzeFrameForProducts(base64);

        for (const product of detectedProducts) {
          const key = product.name.toLowerCase().trim();
          if (
            !allProducts.has(key) ||
            allProducts.get(key)!.confidence < product.confidence
          ) {
            allProducts.set(key, {
              id: uuidv4(),
              name: product.name,
    ...
    try {
      ...
      for (const product of state.products) {
        ...
        if (selection.frameIndex >= 0 && selection.frameIndex < frames.length) {
          ...
          console.log(
            `Selected frame at ${bestFrame.timestamp}s for ${product.name}: ${selection.reason}`
          );
        }
  ...
  async segmentProducts(
    ...
    try {
      ...
      for (const frame of state.extractedFrames) {
        ...
        const segmentResult = await this.geminiClient.segmentProduct(
          frame.frameBase64,
          product.name
        );
  ...
  async enhanceImages(state: GraphStateType): Promise<Partial<GraphStateType>> {
    try {
      if (
        state.error ||
        !state.segmentedImages ||
        state.segmentedImages.length === 0
      ) {
        return {};
      }
